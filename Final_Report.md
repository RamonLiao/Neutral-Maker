...
## 3. 當前運行配置 (Configuration)

為滿足「不長期持倉、賺取微利」的需求，我們部署了 **極速超短線 (HF Scalping)** 模式：

*   **Take Profit (止盈)**: `0.04%` (約 4-5 ticks)。進場後極速離場。
*   **Grid Spread (網格)**: `0.03%`。高密度鋪單，捕捉噪音級別的波動。
*   **Tunnel (首單鉗制)**: `0.02%`。確保永遠有一單貼近盤口，最大化 Maker 成交機率。
