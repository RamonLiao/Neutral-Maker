...
class ASGridTrader:
    ...
    def __init__(self, initial_capital=10000, gamma=0.1, k=1.5, eta=0.001, 
                 inventory_target=0, hedge_threshold=0.2, ucb_arms=[0.05, 0.1, 0.15]):
        """
        初始化交易器。
    ...
    def execute_trade(self, action, price, size):
        ...
        if action == 'buy':
            self.inventory += size
            self.capital -= size * price * 0.01  # 模擬保證金占用 (1% margin)
        else:
            self.inventory -= size
            self.capital += size * price * 0.01  # 釋放保證金
        self.trades.append({'action': action, 'price': price, 'size': size})
